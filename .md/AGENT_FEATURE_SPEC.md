...
- 프롬프트, 진행 표시와 즉시 피드백
- 완료가 확정되는 정확한 시점
- 성공 후 사용자가 관찰할 수 있는 월드 상태와 이용 가능 여부
- 조기 해제, 시선 이탈, 충돌과 비정상 종료 후 관찰 가능한 복원 결과
- 기존 기능 중 반드시 유지할 항목
...
`PROMPT_ARCHITECTURE.md`는 사용자가 승인할 `무엇을` 기록하며, 이를 실현할 `어떻게`는 아키텍처 에이전트가 독점해서 결정한다.

- 목표 구조의 상태·실행·authoring 책임자를 정하거나 추천하지 않는다.
- 새 클래스, Component, Subsystem, Interface, 함수, API와 reflected type을 지정하지 않는다.
- 데이터 표현·저장 구조와 C++/Blueprint 책임 분리를 정하지 않는다.
- Tick, timer, delegate, event, 상태 머신과 StateTree Task/Condition 같은 실행 방식을 정하지 않는다.
- transaction, staging, snapshot, commit, rollback과 migration 방식을 정하지 않는다.
- 구현 파일 목록, 의존 방향과 단계별 내부 호출 순서를 정하지 않는다.
- 기존 기술 이름은 현재 상태의 근거에 꼭 필요할 때만 쓰며 목표 계약으로 승격하지 않는다.
- Given/When/Then은 화면·월드·NPC 행동처럼 외부에서 확인 가능한 결과로 쓰고 함수 호출, delegate 횟수와 내부 lifecycle을 수용 기준으로 삼지 않는다.
- 실패·취소·복구는 원래와 같은 외형·상태·이용 가능성이 보존되는지만 명세한다.
- Editor authoring은 조정 가능 범위·단위·좌표 기준만 명세하고 property type, 저장 위치와 소유 클래스를 정하지 않는다.
- 아키텍처 질문의 답이나 후보 구조를 미리 제시하지 않는다. 설계 에이전트가 Source·정본·사전 조사 결과를 근거로 독립 결정한다.

인계 전에는 각 문장을 `구현 방식이 달라도 동일한 기능 결과를 만족할 수 있는가`로 자체 검토한다. 그렇지 않은 문장은 관찰 가능한 결과로 고치거나 제거한다.
...
- 비목표와 기능 계약에서 고정하지 않는 설계 영역

현재 작업 하나만 기록한다. 줄 수 상한으로 기능 계약을 압축하지 않으며, 중복을 피하되 사용자 승인 사항과 수용 기준의 완결성을 우선한다.
